#!/usr/bin/env python3
"""Generate data-sc.js file with all commands from CSV files - Version 2"""
    ...
    for i, (category, cat_commands) in enumerate(sorted(categories.items())):
        ...
        for cmd in cat_commands:
            ...
            if cmd['bindings']:
                ...
                if hotkey_call:
                    generated_commands.append((hotkey_call, binding['binding']))
        
        # Add generated commands
        for j, (hotkey_call, binding_str) in enumerate(generated_commands):
            js_content += hotkey_call
            if j < len(generated_commands) - 1:
                js_content += ","
            js_content += f"  // {binding_str}\n"
...
'''
    ...
    for category, cat_commands in categories.items():
        ...
        # Special reports
        if category == 'MFD':
            mfd_names = [cmd['name'] for cmd in cat_commands if cmd['bindings']]
            print(f"    MFD commands with bindings: {mfd_names}")
        elif category == 'SOCIAL':
            emote_count = sum(1 for cmd in cat_commands if any(emote in cmd['name'].lower() for emote in ['forward', 'left', 'right', 'stop', 'yes', 'no', 'agree', 'angry', 'at_ease', 'attention', 'blah', 'bow', 'cheer', 'chicken', 'clap', 'come', 'cry', 'dance', 'disagree', 'failure', 'flex', 'flirt', 'gloat', 'greet', 'laugh', 'point', 'salute', 'sit', 'sleep', 'smell', 'taunt', 'threaten', 'wait', 'wave']))
            emotes_with_bindings = sum(1 for cmd in cat_commands if cmd['bindings'] and any(emote in cmd['name'].lower() for emote in ['forward', 'left', 'right', 'stop', 'yes', 'no', 'agree', 'angry', 'at_ease', 'attention', 'blah', 'bow', 'cheer', 'chicken', 'clap', 'come', 'cry', 'dance', 'disagree', 'failure', 'flex', 'flirt', 'gloat', 'greet', 'laugh', 'point', 'salute', 'sit', 'sleep', 'smell', 'taunt', 'threaten', 'wait', 'wave']))
            print(f"    Total emotes: {emote_count}, With bindings: {emotes_with_bindings}")
