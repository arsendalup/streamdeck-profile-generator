#!/usr/bin/env python3
"""Extract all commands from Star Citizen CSV binding files - Version 3"""
...
def parse_binding(binding_str):
    ...
    if any(skip in binding for skip in ['MOUSE', 'JOYSTICK', 'GAMEPAD', 'BUTTON', 'WHEEL', 'MMB']):
        return None
    
    # Skip section headers
    if binding == 'DT= DOUBLE TAP':
        return None
    ...
    # Handle special numpad notation
    binding = binding.replace('NP ', 'NUMPAD ')
    binding = binding.replace('NP DEC', 'NUMPAD DECIMAL')
    binding = binding.replace('NP DECIMAL', 'NUMPAD DECIMAL')
    binding = binding.replace('DT', '')  # Remove double tap notation
    
    # Handle modifier prefixes (L/R modifiers)
    if binding.startswith('L '):
        binding = binding[2:]
    elif binding.startswith('R '):
        binding = binding[2:]
    ...
    if '+' in binding:
        ...
        # Check for space-separated modifiers
        parts = binding.split()
        if len(parts) > 1:
            for part in parts[:-1]:
                if 'CTRL' in part or 'CONTROL' in part:
    ...
    try:
        with open(csv_path, 'r', encoding='utf-8', errors='ignore') as f:
            ...
            for row in reader:
                ...
                if (not row.get('Default Binding', '') and 
                    not row.get('Mine', '') and 
                    not row.get('Alt Mine', '') and
                    'social' not in command_name.lower() and
                    'emote' not in command_name.lower() and
                    'mfd' not in command_name.lower()):
                    continue
                ...
                # Add command even if no bindings for MFD and emotes (they might not have default bindings)
                if bindings or 'MFD' in clean_name or category == 'SOCIAL':
                    commands.append({
                        'name': clean_name,
                        'category': category,
                        'bindings': bindings,
...
def main():
    ...
    for csv_file in csv_files:
        ...
        if os.path.exists(csv_path):
            ...
            # Special report for MFD and SOCIAL
            if csv_file == 'MFD.csv':
                mfd_with_bindings = sum(1 for cmd in commands if cmd['bindings'])
                print(f"    {mfd_with_bindings} MFD commands with keyboard bindings")
            elif csv_file == 'SOCIAL.csv':
                emotes = sum(1 for cmd in commands if 'emote' in cmd['name'].lower() or any(emote in cmd['name'].lower() for emote in ['forward', 'left', 'right', 'stop', 'yes', 'no', 'agree', 'angry', 'at_ease', 'attention', 'blah', 'bow', 'cheer', 'chicken', 'clap', 'come', 'cry', 'dance', 'disagree', 'failure', 'flex', 'flirt', 'gloat', 'greet', 'laugh', 'point', 'salute', 'sit', 'sleep', 'smell', 'taunt', 'threaten', 'wait', 'wave']))
                print(f"    {emotes} emote commands found")
    
    # Save to JSON
    with open('all_sc_commands_v3.json', 'w', encoding='utf-8') as f:
